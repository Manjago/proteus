...
start:                      ; Метка
    ...
    JLT R0, R4, loop       ; Инструкция со ссылкой на метку
...
- **Offset (JMP/JMPZ/JLT):** -131,072 — +131,071 (18 бит, signed)
...
```asm
    ...
    JLT R0, R4, loop   ; Условный переход на "loop"
```
...
| `JLT` | `Ra, Rb, offset` или `label` | if `Ra < Rb` then jump |
...
См. каталог `examples/`:
- `ancestor.asm` — базовый самовоспроизводящийся организм
