...
| `SystemCallHandler` | Интерфейс для ALLOCATE, SPAWN |
...
| `SimulatorStats` | Снимок статистики |
...
| `AnalyzeCommand` | Команда `analyze` — анализ сохранённого запуска |
...
### 9.7. Population Bottleneck (бутылочное горлышко)

**Феномен:** Массовое вымирание, после которого выживает малая группа идентичных особей.
...
Пример запуска:
  ...
  Instruction diff:    0 (все клоны!)
```

**Механизм:**
1. Ранняя популяция растёт, мутации накапливаются
2. Большинство мутаций оказываются летальными
3. Выживают только "чистые" особи без вредных мутаций
4. Популяция восстанавливается из генетически однородной группы

**Последствия:**
- Потеря генетического разнообразия
- Все выжившие — клоны друг друга
- "Чистый лист" для новой эволюции

**Аналог в биологии:** Реальные бутылочные горлышки (гепарды, северные морские слоны) приводят к низкому генетическому разнообразию популяции.
...
Дефолты в `reference.conf`, переопределение через `--config file.conf`:

```hocon
proteus {
  soup.size = 10000           # Размер памяти
  
  mutation {
    rate = 0.002              # 0.2% — оптимальный дефолт
    seed = 0                  # 0 = случайный
  }
  
  reaper {
    strategy = "age-based"
    max-errors = 100          # Смерть после 100 ошибок
  }
  
  limits {
    max-cycles = 0            # 0 = бесконечно
    max-organisms = 1000      # Лимит популяции
  }
  ...
  reporting.interval = 1000   # Прогресс каждые N циклов
}
```

### Эмпирически найденные параметры
...
| 0.3% | Активный | Больше эволюции, лёгкий риск |
| 0.5% | Граничный | Бифуркация — зависит от seed |
| 1.0%+ | Error catastrophe | Вымирание |

**Теоретический порог Эйгена:** `μ_max ≈ 1/L = 1/13 ≈ 7.7%`
**Практический порог:** 0.5%-1.0% (не все биты нейтральны)

#### Соотношение soup/organisms

| Soup size | Max organisms | Обоснование |
|-----------|---------------|-------------|
| 10K | 1,000 | ~100 org при заполнении |
| 100K | 5,000-7,000 | ~7,600 max (100K/13) |
| 1M | 50,000-70,000 | Пропорционально |

**Формула:** `max_organisms ≈ soup_size / genome_size * 0.7`

### Оптимизации производительности

1. **aliveOrganisms[]** — отдельный список живых для O(alive) итерации
2. **Early exit** — `if (aliveCount == 0) return` в runCycle()
3. **INFO logging** — DEBUG только по запросу (`-Dproteus.log.level=DEBUG`)
4. **Population limit** — maxOrganisms предотвращает экспоненциальный рост
...
═══════════════════════════════════════
...
🔑 Seed: 1735645823456 (use --seed 1735645823456 to reproduce)
```
...
└── io/github/manjago/proteus/core/
    ...
    ├── MemoryManagerTest.java — управление памятью (20 тестов)
    └── ReaperTest.java        — жнец (14 тестов)
```
...
### ✅ Выполнено (Stage 3: Application)

- [x] **SimulatorConfig** — HOCON конфигурация (typesafe-config)
- [x] **Simulator** — основной движок симуляции
- [x] **CLI** — picocli (run, analyze, info)
- [x] **Graceful shutdown** — обработка Ctrl+C
- [x] **Random seed** — воспроизводимость запусков
- [x] **Population limit** — maxOrganisms для контроля роста
- [x] **Оптимизация O(alive)** — aliveOrganisms[] для быстрой итерации
- [x] **Эмпирические параметры** — найдены оптимальные mutation rates

### 🚧 В работе (Stage 3: Persistence)
...
- [ ] Команда `resume` — продолжение с checkpoint'а
- [ ] Команда `analyze` — анализ сохранённого запуска

### 📋 Планы (Stage 4: Analytics)

- [ ] Genealogy tracking (родословные)
- [ ] Diversity metrics (уникальные геномы, Hamming distance)
- [ ] Дефрагментация памяти
- [ ] Export в CSV/JSON

### 📋 Планы (Stage 5+: Advanced)

- [ ] Web UI для визуализации
- [ ] Параллельное исполнение (многопоточность)
- [ ] Energy-based scheduling
- [ ] Мультиплеер (деплой организмов игроками)
